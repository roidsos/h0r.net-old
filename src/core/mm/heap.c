...
#define MAX_PAGES (4096 * SLAB_PAGES_TO_STORE_FUCKING_PAGES_WITH) / sizeof(page_t)
...
void heap_init(){
    pages = (page_t *)PHYS_TO_VIRT(request_pages(SLAB_PAGES_TO_STORE_FUCKING_PAGES_WITH));
    num_used_pages = 0;
...
void *malloc(size_t size) {
    ...
    if(!curpage->paddr) // needing this redundant check, since we use curpage->paddr to calcualte PP
...
void free(void *tofree) {
    ...
    if(hdata.page->fullness > old_fullness){// checking whether it went bellow 0 via overflow check(since size_t is unsigned)

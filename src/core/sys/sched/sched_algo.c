...
void next_process() {
    ...
    if (sched_current_pid > sched_num_procs)
        sched_current_pid = 0;
    while (!(SCHED_GET_STATE(processes[sched_current_pid].state_flags) == SCHED_STATE_READY)) {
        // WARN: Arch specific code in core/ + TODO: smh move to arch/ or make
        // architecture neutral
        if (SCHED_GET_STATE(processes[sched_current_pid].state_flags) == SCHED_STATE_DEAD) {

    ...
    for(size_t i = 0; i < num_madt_entries;i++){
        if(entry_ptrs[i]->h.type == MADT_ENTRY_TYPE_IOAPIC
                && entry_ptrs[i]->the_meat.ioapic.GS_interrupt_base <= gsi
                    && entry_ptrs[i]->the_meat.ioapic.GS_interrupt_base + vector_count > gsi ){
            return entry_ptrs[i];
        }
    }
    return NULL;
...
void mask(uint8_t id){
    ...
    for(size_t i = 0; i < num_madt_entries;i++){
        if(entry_ptrs[i]->h.type == MADT_ENTRY_TYPE_IOAPIC_OVERRIDE
            ...
            madt_entry* gsi = get_gsi(entry_ptrs[i]->the_meat.ioapic_override.GS_interupt);
            void* tmp = ioapicaddr;
            ioapicaddr = (void*)(uint64_t)gsi->the_meat.ioapic.adress;

            uint64_t redir = id;

            if ((entry_ptrs[i]->the_meat.ioapic_override.flags & (1 << 1)) != 0) {
                redir |= (1 << 13);
            }
...
void unmask(uint8_t id){
    ...
    for(size_t i = 0; i < num_madt_entries;i++){
        if(entry_ptrs[i]->h.type == MADT_ENTRY_TYPE_IOAPIC_OVERRIDE
            ...
            write((entry_ptrs[i]->the_meat.ioapic_override.GS_interupt - gsi->the_meat.ioapic.GS_interrupt_base) * 2 + 16, (uint32_t) redir);
            write((entry_ptrs[i]->the_meat.ioapic_override.GS_interupt - gsi->the_meat.ioapic.GS_interrupt_base) * 2 + 17, (uint32_t)(redir >> 32));
            ioapicaddr = tmp;
            return;
        }

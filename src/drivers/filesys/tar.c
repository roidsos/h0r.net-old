...
struct node tar_inspect(char *path) {
    ...
    if (header->typeflag == '5') {
        ...
        node_.ext.ext_dir.last_modified_unix_time = parse_size(header->mtime);
        node_.ext.ext_dir.creation_date_unix_time = parse_size(header->mtime);
        ...
        node_.ext.ext_file.last_modified_unix_time = parse_size(header->mtime);
        node_.ext.ext_file.creation_date_unix_time = parse_size(header->mtime);
        node_.ext.ext_file.size = parse_size(header->size);
...
struct file_buffer tar_read_file(char *path, uint64_t offset, uint64_t size) {
    ...
    return (struct file_buffer){((char *)thdr) + 512 + offset,
                                size > fsize ? fsize : size};
}

bool tar_write_file(__attribute__((unused)) char *path,
                    __attribute__((unused)) uint64_t offset,
                    __attribute__((unused)) struct file_buffer buf) {
    return 0;
}
...
struct dir_report tar_iterate_dir(char *path) {
    ...
    for (size_t i = 0; i < data.initramfs.hdr_num; i++) {
        ...
        if (strncmp(path2 + 1, (char *)hdr->filename, strlen(path2 + 1)) == 0 &&
            ...
            (!contains((char *)hdr->filename + strlen(path2), '/') ||
             hdr->filename[strlen((char *)hdr->filename) - 1] ==
                 '/')) { // check if starts with the
                         // path but not the same
            vector_push_back(&nodes, tar_inspect((char *)hdr->filename));
        }
    }
...
void tar_init() {
    struct fs_driver driver = {true, tar_inspect, tar_iterate_dir,
                               tar_read_file, tar_write_file};

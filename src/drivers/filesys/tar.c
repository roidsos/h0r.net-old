...
struct node *tar_get_file_header(char *path) {
    ...
    if (strcmp((char *)header->mode, "0000755")) {
        ...
        ((struct ext_dir *)node_->ext)->last_modified_unix_time =
            parse_size(header->mtime);
        ((struct ext_dir *)node_->ext)->creation_date_unix_time =
            parse_size(header->mtime);
        ...
        ((struct ext_file *)node_->ext)->last_modified_unix_time =
            parse_size(header->mtime);
        ((struct ext_file *)node_->ext)->creation_date_unix_time =
            parse_size(header->mtime);
        ((struct ext_file *)node_->ext)->contents_on_disk_ptr =
            0; // unused since tar is not a disk
...
struct node *tar_get_dir_contents(char *path) {
    ...
    for (size_t i = 0; i < data.initramfs.hdr_num; i++) {
        ...
        if (strncmp(path, (char *)hdr->filename, strlen(path)) &&
            !strcmp(path, (char *)hdr->filename)) { // check if starts with the
                                                    // path but not the same
            vector_push_back(&nodes,
                             *tar_get_file_header((char *)hdr->filename));
        }
    }
...
void tar_init() {
    struct fs_driver driver = {true, tar_get_file_header, tar_get_dir_contents,
                               tar_get_file_contents};

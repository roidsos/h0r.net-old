...
void sb_play(int freq) {
    ...
    outb8(0X0c, 1);
    outb8(0x0B, 0x49);
    outb8(0x83, ((uint64_t)VIRT_TO_PHYS(buf) >> 16) & 0xff); //PAGE TRANSFER (EXAMPLE POSITION IN MEMORY 0x[01]0F04) - SET THIS VALUE FOR YOU
    outb8(0x02, ((uint64_t)VIRT_TO_PHYS(buf) >> 8) & 0xff); //POSITON HIGH BIT (EXAMPLE POSITION IN MEMORY 0x01[0F]04) - SET THIS VALUE FOR YOU
    outb8(0x02, (uint64_t)VIRT_TO_PHYS(buf) & 0xff); //POSITION LOW BIT (EXAMPLE POSITION IN MEMORY 0x010F[04]) - SET THIS VALUE FOR YOU
    outb8(0x03, 0xFF);
    outb8(0x03, 0x0F);
    outb8(0x0A, 1);

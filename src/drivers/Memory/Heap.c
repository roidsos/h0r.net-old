    ...
    while (true) {
        if (current->length >= size) {
            if (current->length > size + sizeof(struct MemorySegment)) {
                ...
                newSegment->length = (((uint64_t)current->length) -
                                      sizeof(struct MemorySegment) + size);
                ...
                current->nextfree =
                    newSegment; // setting current's next segment
            ...
            if (current->previousfree != 0)
                current->previousfree->nextfree =
                    current->nextfree; // skipping the current backwards for the
                                       // frees
            if (current->nextfree != 0)
                current->nextfree->previousfree =
                    current->previousfree; // skipping the current forwards for
                                           // the frees
...
void free(void *tofree) {
    ...
    if (ams->IsAligned) {
        current =
            (struct MemorySegment *)(uint64_t)ams
                ->MemorySegmentAddr; // establishing the target but its aligned
    } else {
        current =
            ((struct MemorySegment *)tofree) - 1; // establishing the target
    }
    ...
    if (current->next != 0) {
        if (current->next->previous < current) {
            ...
            if (current->next->isfree)
                CombineSegments(current,
                                current->next); // removing fragmentation
        }
    }
    if (current->previous != 0) {
...
void *aligned_alloc(uint64_t alignment, uint64_t size) {
    ...
    if (remainder2 != 0) {
        ...
        struct AlignedMemorySegment *ams =
            (struct AlignedMemorySegment *)address -

    ...
    while(true){
        if (current->length >= size){
            if (current->length > size + sizeof(MemorySegment)){
                MemorySegment* newSegment = (MemorySegment*)((uint64_t)current + sizeof(MemorySegment) + size);//making a new segment
                newSegment->isfree            = true;//giving it values
                newSegment->length          = (((uint64_t)current->length) - sizeof(MemorySegment) +size);
...
void free(void* tofree){
    ...
    if(ams->IsAligned){
    current = (MemorySegment*)(uint64_t)ams->MemorySegmentAddr;//establishing the target but its aligned
    }
    else{
...
void* realloc(void* old,uint64_t size){
    ...
    if(ams->IsAligned){
        oldseg = (MemorySegment*)(uint64_t)ams->MemorySegmentAddr;//establishing the target but its aligned
    }
    else{

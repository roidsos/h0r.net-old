    ...
    while(true){
        if (current->length >= size){
            if (current->length > size + sizeof(struct MemorySegment)){
                struct MemorySegment* newSegment = (struct MemorySegment*)((uint64_t)current + sizeof(struct MemorySegment) + size);//making a new segment
                newSegment->isfree            = true;//giving it values
                newSegment->length          = (((uint64_t)current->length) - sizeof(struct MemorySegment) +size);
...
void free(void* tofree){
    ...
    register struct AlignedMemorySegment* ams = ((struct AlignedMemorySegment*)tofree) - 1;
    if(ams->IsAligned){
    current = (struct MemorySegment*)(uint64_t)ams->MemorySegmentAddr;//establishing the target but its aligned
    }
    else{
...
void* realloc(void* old,uint64_t size){
    ...
    register struct AlignedMemorySegment* ams = ((struct AlignedMemorySegment*)old) - 1;
    if(ams->IsAligned){

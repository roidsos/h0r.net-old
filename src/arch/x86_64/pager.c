...
uint64_t create_pagetable() { return 0; }
...
bool map_page(UNUSED uint64_t vaddr, UNUSED uint64_t paddr,
              UNUSED uint64_t flags) {
    return true;
}
bool unmap_page(UNUSED uint64_t vaddr) { return true; }
...
uint64_t phys_to_virt(UNUSED uint64_t paddr, UNUSED uint64_t pml4) { return 0; }

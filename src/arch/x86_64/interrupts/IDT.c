...
void SetGate(int interrupt, void* base, uint16_t segmentDescriptor, uint8_t flags)
{
    ...
    g_IDT[interrupt].offset_low  = (uint16_t)(((uint64_t)base & 0x000000000000ffff));
    g_IDT[interrupt].offset_mid  = (uint16_t)(((uint64_t)base & 0x00000000ffff0000) >> 16);
    g_IDT[interrupt].offset_high = (uint32_t)(((uint64_t)base & 0xffffffff00000000) >> 32);
